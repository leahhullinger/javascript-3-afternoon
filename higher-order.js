...
//Code Here
const postTaxPrices = prices.map(origPrice => {
  return origPrice * 1.07;
}, 0);
...
//Code Here
const myStrongest = monstersInYourPocket.filter(value => value.CP > 200);
...
let ordersTotal = orders.map(order => order.price * order.tax + order.price);
